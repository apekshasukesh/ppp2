...
class Test_ruffus(unittest.TestCase):
    ...
    def test_newstyle_simpler1 (pipeline_name):
        print ("output from pipeline1")
        test_pipeline = Pipeline(pipeline_name)
        #test_pipeline.start()
        test_pipeline.transform(start, name = 'test:start', filter=suffix(".vcf.gz"),output = ".recode.bcf")
        test_pipeline.transform(run_vcf_to_filter, name = 'test:run_vcf_to_filter', filter = suffix(".recode.bcf"),output = ".windowed.weir.fst")
        tail_task =  test_pipeline.transform(run_vcf_to_calc, name = 'test:run_vcf_to_calc', filter = suffix(".windowed.weir.fst"),output = ".tsv")
        test_pipeline.set_tail_tasks([tail_task])

        if DEBUG_do_not_define_tail_task:
           test_pipeline.set_tail_tasks([tail_task])
 
        test_pipeline.set_head_tasks([test_pipeline[start]])

        return test_pipeline

    pipeline1a = test_newstyle_simpler1(pipeline_name = "pipeline1a")
    pipeline1b = test_newstyle_simpler1(pipeline_name = "pipeline1b")
    ...
    pipeline1a.run()
        ...
        #test_pipeline.run(multiprocess = 4, verbose = 0)
        #test_pipeline.run()
